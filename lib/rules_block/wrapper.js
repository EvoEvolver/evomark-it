...
module.exports = function getWrapperParser(tokenizeContainer, tokenizeCommand) {
    function parseWrapper(state, startLine, endLine, silent) {
        ...
        const nestingLevelInit = nestingLevel
        if (!single_line) {
            for (; ;) {
                currentLine++;
                ...
                for (pos = start + 1; pos <= max; pos++) {
                    if (counting_str !== state.src[pos]) {
                        if (!isInnerFence) {
                            if (state.src[pos] == marker_starter || state.src[pos] == marker_closer) { pos++ }
                        }
                        break;
                    }
                }
                ...
                if (state.src[pos-1] == marker_starter) {
                    nestingLevel++
                    continue
                } else if (state.src[pos-1] == marker_closer) {
                    nestingLevel--
                    if (!hasStarterMark)
                        continue
                }
                ...
                if (!isInnerFence && nestingLevel == 0) {
                    // end found!
                    auto_closed = true;
                    break;
                }
                // Fence only happen when the nesting level go back to its initial value
                if (isInnerFence && nestingLevel == nestingLevelInit) {
