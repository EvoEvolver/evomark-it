...
function renderCaptioned(token) {
    if (token.fenceIndex[0] == 0) {
        if (token.nesting === 1) {
            let attrs = token.attrs
            let name = attrs.tagName || token.tag
            return writeOpen('RemarkBox', { id: attrs.id, name: name, index: attrs.index, title:attrs.title })
        } else {
            return "";
        }
    }
    if (token.fenceIndex[0] == 1) {
        if (token.nesting === 1) {
            return writeOpen("template", {"v-slot:caption":null })
        } else {
